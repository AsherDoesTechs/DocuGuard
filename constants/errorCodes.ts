export const ErrorCodes = {
  ...
  // Local storage errors (4xxx)
  LOCAL_DB_INIT: 4001,
  LOCAL_DB_QUERY: 4002,
  LOCAL_WRITE_FAILED: 4003,
  LOCAL_READ_FAILED: 4004,
  FILE_SAVE_FAILED: 4005,
...
export function getErrorMessage(code: number): string {
  const messages: Record<number, string> = {
    ...
    [ErrorCodes.LOCAL_DB_INIT]: "Local database initialization failed.",
    [ErrorCodes.LOCAL_DB_QUERY]: "Local database query failed.",
    [ErrorCodes.LOCAL_WRITE_FAILED]: "Failed to write to local storage.",
    [ErrorCodes.LOCAL_READ_FAILED]: "Failed to read from local storage.",
    [ErrorCodes.FILE_SAVE_FAILED]: "Failed to save file locally.",
    ...
    [ErrorCodes.NETWORK_OFFLINE]: "You appear to be offline. Some features may be limited.",
...
export function formatError(code: number, detail?: string): string {
  const message = getErrorMessage(code);
  const baseMessage = `(Code: ${code}) ${message}`;
